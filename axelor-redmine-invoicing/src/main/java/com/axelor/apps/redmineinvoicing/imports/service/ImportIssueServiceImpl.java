 ...
 */
package com.axelor.apps.redmineinvoicing.imports.service;
...
public class ImportIssueServiceImpl extends ImportService implements ImportIssueService {
  ...
  public TeamTask getTeamTask(Issue redmineIssue, Date lastImportDateTime) {
  ...
  private void importRedmineIssue(
      ...
      Consumer<Object> onSuccess,
      Consumer<Throwable> onError) {
    TeamTask teamTask = getTeamTask(redmineIssue, lastImportDateTime);
    if (teamTask != null) {
      try {
  ...
  private void setTeamTaskProjectPlanningTimeList(
    ...
    try {
      ...
      for (TimeEntry timeEntry : timeEntries) {
        if (lastImportDateTime != null && timeEntry.getCreatedOn().before(lastImportDateTime)) {
          continue;
        }
