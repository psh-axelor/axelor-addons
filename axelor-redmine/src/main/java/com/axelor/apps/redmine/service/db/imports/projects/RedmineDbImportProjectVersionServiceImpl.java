...
public class RedmineDbImportProjectVersionServiceImpl extends RedmineDbImportCommonService {
  ...
  public void importRedmineProjectVersions(Connection connection, LocalDateTime lastBatchEndDate) {
    ...
    try (PreparedStatement preparedStatement =
      ...
      ResultSet projectVersionResultSet = preparedStatement.executeQuery();

      while (projectVersionResultSet.next()) {
        updateTransaction();
        offset = processProjectVersionResultSet(projectVersionResultSet, lastBatchEndDate, offset);
        preparedStatement.setInt(1, offset);
  ...
  public Integer processProjectVersionResultSet(
      ResultSet projectVersionResultSet, LocalDateTime lastBatchEndDate, Integer offset)
      throws SQLException {
  ...
  public ProjectVersion importRedmineProjectVersion(
    ...
    ProjectVersion projectVersion =
        projectVersionRepo
            .all()
            .filter("self.redmineId = ?1", redmineProjectVersionId)
            .autoFlush(false)
            .fetchOne();
    ...
    Project project =
        projectRepo
            .all()
            .filter("self.redmineId = ?1", projectVersionResultSet.getInt("project_id"))
            .autoFlush(false)
            .fetchOne();
      ...
      switch (projectVersionResultSet.getString("sharing")) {
        ...
        case "system":
          projectVersion.setProjectSet(
              new HashSet<>(
                  projectRepo.all().filter("self.redmineId != 0").autoFlush(false).fetch()));
          break;
        case "descendants":
