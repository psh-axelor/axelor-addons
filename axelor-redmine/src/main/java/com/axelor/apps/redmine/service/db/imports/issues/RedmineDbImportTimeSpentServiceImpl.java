...
public class RedmineDbImportTimeSpentServiceImpl extends RedmineDbImportCommonService
  ...
  public void importRedmineTimeSpents(
    ...
    try (PreparedStatement preparedStatement = connection.prepareStatement(timeEntryQuery)) {
      ...
      ResultSet timeSpentResultSet = preparedStatement.executeQuery();

      while (timeSpentResultSet.next()) {
        updateTransaction();
        offset = processTimeSpentResultSet(timeSpentResultSet, lastBatchEndDate, offset);
        preparedStatement.setInt(1, offset);
  ...
  public Integer processTimeSpentResultSet(
      ResultSet timeSpentResultSet, LocalDateTime lastBatchEndDate, Integer offset)
      throws SQLException {
    ...
    do {
      ...
      try {
        ...
        TimesheetLine timesheetLine =
            importRedmineTimeSpent(lastBatchEndDate, timeSpentResultSet, redmineUpdatedOn);
        ...
        failedIds =
            StringUtils.isEmpty(failedIds)
                ...
                : failedIds + "," + timeSpentResultSet.getInt("id");
        onError.accept(e);
        TraceBackService.trace(e, "", batch.getId());
      }
  ...
  public TimesheetLine importRedmineTimeSpent(
      LocalDateTime lastBatchEndDate, ResultSet timeSpentResultSet, LocalDateTime redmineUpdatedOn)
      throws SQLException, JSONException, AxelorException {

    int redmineTimeSpentId = timeSpentResultSet.getInt("id");
    ...
    if (errors.length > 0) {
      ...
      setErrorLog(IMessage.REDMINE_IMPORT_TIMESHEET_LINE_ERROR, String.valueOf(redmineTimeSpentId));
      return null;
    }
    ...
    TimesheetLine timesheetLine =
        timesheetLineRepo
            .all()
            .filter("self.redmineId = ?1", redmineTimeSpentId)
            .autoFlush(false)
            .fetchOne();
  ...
  public void validateRequiredFields(ResultSet timeSpentResultSet) throws SQLException {
    ...
    if (userRepo
            ...
            .filter(
                ...
                timeSpentResultSet.getString("user"))
            .autoFlush(false)
            .count()
        == 0) {
    ...
    if (projectRepo
            .all()
            .filter("self.redmineId = ?1", timeSpentResultSet.getInt("project_id"))
            .autoFlush(false)
            .count()
        == 0) {
      errors =
    ...
    if (issueId != 0
        && teamTaskRepo.all().filter("self.redmineId = ?1", issueId).autoFlush(false).count()
            == 0) {
      errors =
          errors.length == 0
  ...
  public TimesheetLine setTimesheetLineFields(
    ...
    Integer issueId = timeSpentResultSet.getInt("issue_id");
    timesheetLine.setTeamTask(
        issueId != 0
            ? teamTaskRepo.all().filter("self.redmineId = ?1", issueId).autoFlush(false).fetchOne()
            : null);
        ...
        if (cfName.equals(appRedmine.getRedmineTimeSpentProduct())) {
          ...
          Product product =
              !StringUtils.isEmpty(cfValue)
                  ? productRepo.all().filter("self.code = ?1", cfValue).autoFlush(false).fetchOne()
                  : null;
          ...
          if (!StringUtils.isEmpty(cfValue)) {
            unit = unitRepo.all().filter("self.name = ?1", cfValue).autoFlush(false).fetchOne();
          }
          ...
          if (unit == null) {
            unit =
                unitRepo
                    .all()
                    .filter("self.name = ?1", appRedmine.getRedmineTimeSpentDurationUnitDefault())
                    .autoFlush(false)
                    .fetchOne();
