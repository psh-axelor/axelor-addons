...
public class RedmineDbImportIssueJournalServiceImpl extends RedmineDbImportCommonService {
  ...
  public void importIssueJournals(Connection connection) {
    ...
    try (PreparedStatement preparedStatement =
      ...
      ResultSet journalResultSet = preparedStatement.executeQuery();

      while (journalResultSet.next()) {
        updateTransaction();
        offset = processJournalResultSet(journalResultSet, offset, connection);
        preparedStatement.setInt(1, offset);
  ...
  @Transactional
  public Integer processJournalResultSet(
      ResultSet journalResultSet, Integer offset, Connection connection) throws SQLException {
        ...
        MailMessage mailMessage =
            createMailMessage(
                ...
                getDateAtLocalTimezone(
                    journalResultSet.getObject("created_on", LocalDateTime.class)),
  ...
  public String getProjectName(String value) {
    ...
    if (!projectMap.containsKey(value)) {
      Project project =
          projectRepo
              .all()
              .filter("self.redmineId = ?1", Integer.parseInt(value))
              .autoFlush(false)
              .fetchOne();
