...
public class RedmineDbImportProjectServiceImpl extends RedmineDbImportProjectVersionServiceImpl
  ...
  public void importRedmineProjects(Connection connection, LocalDateTime lastBatchEndDate) {
    ...
    try (PreparedStatement preparedStatement =
      ...
      ResultSet projectResultSet = preparedStatement.executeQuery();

      while (projectResultSet.next()) {
        updateTransaction();
        offset = processProjectResultSet(projectResultSet, lastBatchEndDate, offset);
        preparedStatement.setInt(1, offset);
  ...
  public Integer processProjectResultSet(
      ResultSet projectResultSet, LocalDateTime lastBatchEndDate, Integer offset)
      throws SQLException {
    ...
    do {
      ...
      try {
        ...
        Project project =
            importRedmineProject(lastBatchEndDate, projectResultSet, redmineUpdatedOn);
  ...
  public Project importRedmineProject(
      LocalDateTime lastBatchEndDate, ResultSet projectResultSet, LocalDateTime redmineUpdatedOn)
      throws SQLException, JSONException {

    Project project =
        projectRepo
            .all()
            .filter("self.redmineId = ?1", projectResultSet.getInt("id"))
            .autoFlush(false)
            .fetchOne();

    if (project == null) {
      project = new Project();
      project.setCode(projectResultSet.getString("identifier").toUpperCase());
      ...
      for (String trackerName : trackersNameArray) {
        TeamTaskCategory projectCategory =
            teamTaskCategoryRepo
                .all()
                .filter("self.name = ?1", fieldMap.get(trackerName))
                .autoFlush(false)
                .fetchOne();
        ...
        if (cfName.equals(appRedmine.getRedmineProjectInvoiceable())) {
          ...
          if (!StringUtils.isEmpty(cfValue)) {
            Partner partner =
                partnerRepo
                    .all()
                    .filter("self.partnerSeq = ?1", cfValue)
                    .autoFlush(false)
                    .fetchOne();
    ...
    for (Map.Entry<Long, Integer> entry : parentMap.entrySet()) {
      ...
      if (parentProjectMap.containsKey(entry.getValue())) {
        ...
        parentProject =
            projectRepo
                .all()
                .filter("self.redmineId = ?1", entry.getValue())
                .autoFlush(false)
                .fetchOne();
