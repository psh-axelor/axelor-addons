...
public class RedmineDbImportIssueServiceImpl extends RedmineDbImportIssueJournalServiceImpl
  ...
  public void importRedmineIssues(
    ...
    try (PreparedStatement preparedStatement = connection.prepareStatement(issueQuery)) {
      ...
      while (issueResultSet.next()) {
        updateTransaction();
        offset = processIssueResultSet(issueResultSet, lastBatchEndDate, offset);
  ...
  public Integer processIssueResultSet(
      ResultSet issueResultSet, LocalDateTime lastBatchEndDate, Integer offset)
      throws SQLException {
    ...
    do {
      ...
      try {
        ...
        TeamTask teamTask = importRedmineIssue(lastBatchEndDate, issueResultSet, redmineUpdatedOn);
        ...
        if (teamTask != null) {
          ...
          if (batch.getRedmineBatch().getIsImportIssuesWithActivities()) {
            ...
            redmineIssueIdMap.put(issueResultSet.getInt("id"), redmineIssueArray);
          }
        ...
        failedIds =
            StringUtils.isEmpty(failedIds)
                ? String.valueOf(issueResultSet.getInt("id"))
                : failedIds + "," + issueResultSet.getInt("id");
        onError.accept(e);
        TraceBackService.trace(e, "", batch.getId());
      }
  ...
  public TeamTask importRedmineIssue(
      LocalDateTime lastBatchEndDate, ResultSet issueResultSet, LocalDateTime redmineUpdatedOn)
      throws SQLException, JSONException {

    int redmineIssueId = issueResultSet.getInt("id");
    ...
    TeamTask teamTask =
        teamTaskRepo
            .all()
            .filter("self.redmineId = ?1", redmineIssueId)
            .autoFlush(false)
            .fetchOne();
  ...
  public void validateRequiredFields(ResultSet issueResultSet) throws SQLException {
    ...
    if (projectRepo
            .all()
            .filter("self.redmineId = ?1", issueResultSet.getInt("project_id"))
            .autoFlush(false)
            .count()
        == 0) {
    ...
    if (teamTaskCategoryRepo
            .all()
            .filter("self.name = ?1", fieldMap.get(issueResultSet.getString("tracker")))
            .autoFlush(false)
            .count()
        == 0) {
      errors =
    ...
    if (!StringUtils.isEmpty(value)
        && productRepo.all().filter("self.code = ?1", value).autoFlush(false).count() == 0) {
      errors =
          errors.length == 0
  ...
  public TeamTask setTeamTaskFields(TeamTask teamTask, ResultSet issueResultSet)
    ...
    if (issueResultSet.getInt("parent_id") == 0) {
    ...
    if (appBaseService.isApp("business-support")) {
      ...
      if (fixedVersionId != 0) {
        targetVersion =
            projectVersionRepo
                .all()
                .filter("self.redmineId = ?1", fixedVersionId)
                .autoFlush(false)
                .fetchOne();
    ...
    if (!teamTask.getInvoiced()) {
      ...
      teamTask.setProduct(
          !StringUtils.isEmpty(value)
              ? productRepo.all().filter("self.code = ?1", value).autoFlush(false).fetchOne()
              : null);
